...
<body class="bg-gray-100"<#if session?has_content> data-session="${session.id}"</#if>>
                                  ...
                                  <tr data-resource="${result.getLeft().getPath()}" data-resource-visibility="${result.getLeft().getVisibility()}" data-resource-favorite="${result.getLeft().getFavorite()?then('true','false')}">
                                      ...
                                      <td class="align-middle d-none d-lg-table-cell">
                                          <#if result.getLeft().getHtmlDescription()?has_content>
                                              <span title="${result.getLeft().getHtmlDescription()}" data-clipboard-text="${result.getLeft().getHtmlDescription()}">${result.getLeft().getHtmlDescription()?truncate(70, '...')}</span>
                                          <#else>
